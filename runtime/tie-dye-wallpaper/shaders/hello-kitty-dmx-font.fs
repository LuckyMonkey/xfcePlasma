...
float tracedCat(vec2 p, float beat) {
    ...
    // Dress, raised arms, legs, and tail.
...
int glyphRow(int kind, int row) {
    // Five-column, seven-row fallback glyphs: @ # $ & * +.
    if (kind == 0) {
        int rows[7] = int[7](14, 17, 23, 21, 23, 16, 15);
        return rows[row];
    }
    if (kind == 1) {
        int rows[7] = int[7](10, 31, 10, 10, 31, 10, 10);
        return rows[row];
    }
    if (kind == 2) {
        int rows[7] = int[7](14, 20, 30, 5, 15, 5, 30);
        return rows[row];
    }
    if (kind == 3) {
        int rows[7] = int[7](12, 18, 20, 8, 21, 18, 13);
        return rows[row];
    }
    if (kind == 4) {
        int rows[7] = int[7](4, 21, 14, 31, 14, 21, 4);
        return rows[row];
    }
    int rows[7] = int[7](4, 4, 4, 31, 4, 4, 4);
    return rows[row];
}

float fallbackGlyph(vec2 p, int kind) {
    vec2 uv = p / vec2(0.24, 0.24) + 0.5;
    if (uv.x < 0.0 || uv.x >= 1.0 || uv.y < 0.0 || uv.y >= 1.0) return 0.0;

    vec2 grid = vec2(uv.x * 5.0, (1.0 - uv.y) * 7.0);
    int col = clamp(int(floor(grid.x)), 0, 4);
    int row = clamp(int(floor(grid.y)), 0, 6);
    int mask = glyphRow(kind, row);
    float enabled = float((mask >> (4 - col)) & 1);
...
float glyph(vec2 p, float kindValue) {
    int kind = clamp(int(floor(kindValue + 0.5)), 0, 5);

    // A valid atlas has a black border. If the sampler is unbound, raylib's
    // default white texture would otherwise turn every particle into a block.
    float atlasBackground = texture(glyphAtlas, vec2(0.01, 0.01)).r;
    float atlasValid = 1.0 - smoothstep(0.08, 0.20, atlasBackground);

    float cached = atlasGlyph(p, kind);
    float builtIn = fallbackGlyph(p, kind);
    return mix(builtIn, cached, atlasValid);
}
...
void main() {
    ...
    col += vec3(1.0, 0.01, 0.42) *
        (exp(-abs(uv.y - scanY) * 260.0) + 0.1 * exp(-abs(uv.y - scanY) * 28.0));

    // Sparse laser-wall glyphs. Cached atlas glyphs are used when available;
    // otherwise the built-in 5x7 glyphs keep the effect fully functional.
    for (int i = 0; i < 10; i++) {
        float fi = float(i);
        vec2 gp = vec2(
            mod(-1.1 + fi * 0.43 - t * (0.12 + 0.02 * mod(fi, 3.0)), 2.2) - 1.1,
            0.38 + fract(fi * 0.417) * 0.28);
        vec2 local = (uv - gp) / 0.95;
        float g = floor(hash(vec2(fi, 7.0)) * 6.0);
        col += glyph(local, g) * vec3(1.0, 0.01, 0.45) * 3.0;
    }

    // Alternating inverse checkerboard floor.
    if (uv.y < -0.08) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        float cat = tracedCat((uv - vec2(x, -0.02)) / 0.42, beat);
        col += cat * vec3(1.0, 0.005, 0.48) * (0.8 + 0.2 * sin(t * 8.0));
    }
