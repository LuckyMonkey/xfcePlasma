...
service="$repo_root/systemd/user/tie-dye-wallpaper-mvp.service"
...
require 'Restart=on-failure' "$service" 'wallpaper service crash recovery disappeared'
require 'KillMode=control-group' "$service" 'wallpaper service no longer owns its backend process group'
